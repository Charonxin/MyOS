...
/* 击活页表 */
void
page_dir_activate (struct task_struct* p_thread) {
  /********************************************************
   * 执行此函数时,当前任务可能是线程。
  ...
  uint32_t pagedir_phy_addr=
      0x100000; // 默认为内核的页目录物理地址,也就是内核线程所用的页目录表
...
void
process_activate (struct task_struct* p_thread) {
...
/* 创建用户进程虚拟地址位图 */
void
create_user_vaddr_bitmap (struct task_struct* user_prog) {
...
/* 创建用户进程 */
void
process_execute (void* filename, char* name) {
