...
CC = gcc
LD = ld
LIB = -I lib/ -I kernel/ -I device/ -I lib/kernel/ -I thread/ -I userprog/
ASFLAGS = -f elf
ASIB = -I boot/include/
...
OBJS = $(BUILD_DIR)/main.o $(BUILD_DIR)/init.o $(BUILD_DIR)/interrupt.o $(BUILD_DIR)/timer.o $(BUILD_DIR)/kernel.o \
	 ...
	 $(BUILD_DIR)/syscall-init.o $(BUILD_DIR)/stdio.o $(BUILD_DIR)/ide.o $(BUILD_DIR)/stdio-kernel.o
...
$(BUILD_DIR)/ide.o: device/ide.c device/ide.h lib/stdint.h thread/sync.h lib/kernel/list.h kernel/global.h thread/thread.h lib/kernel/bitmap.h \
     	kernel/memory.h lib/kernel/io.h lib/stdio.h lib/stdint.h lib/kernel/stdio-kernel.h kernel/interrupt.h kernel/debug.h device/console.h device/timer.h lib/string.h
	$(CC) $(CFLAGS) $< -o $@
