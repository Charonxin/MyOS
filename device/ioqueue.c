...
void
ioqueue_init (struct ioqueue* queue) {
...
static int32_t
next_pos (int32_t pos) {
  return (pos + 1) % buf_size;
}

int
is_queue_full (struct ioqueue* queue) {
  return next_pos (queue->head) == queue->tail;
}

int
is_queue_empty (struct ioqueue* queue) {
  return queue->head == queue->tail;
}

static void
queue_wait (struct task_struct** waiter) {
  *waiter= running_thread ();
  thread_block (TASK_BLOCKED);
}

static void
wakeup (struct task_struct** waiter) {
 ...
 */
char
queue_getchar (struct ioqueue* queue) {
...
char
queue_putchar (struct ioqueue* queue, char byte) {
  ASSERT (intr_get_status () == INTR_OFF);
