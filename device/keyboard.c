...
init_keyboard_handler (void) {
  ...
  if ((code <= 0x00 || code > 0x3a) &&
      (code != alt_r_make && code != ctrl_r_make)) {
    put_str ("Unkown key.\n");
    return;
  }
  ...
  if (code < 0x0e || code == 0x29 || code == 0x1a || code == 0x1b ||
      code == 0x2b || code == 0x27 || code == 0x28 || code == 0x33 ||
      code == 0x34 || code == 0x35) {
    // 0-9, [等字符不关心caps
    shift= shift_status;
  }
