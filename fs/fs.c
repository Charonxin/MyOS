...
static bool
mount_partition (struct list_elem* pelem, int arg) {
...
static void
partition_format (struct partition* part) {
  /* 为方便实现,一个块大小是一扇区 */
...
/* 将最上层路径名称解析出来 */
static char*
path_parse (char* pathname, char* name_store) {
...
static int
search_file (const char* pathname, struct path_search_record* searched_record) {
...
int32_t
sys_open (const char* pathname, uint8_t flags) {
...
/* 将文件描述符转化为文件表的下标 */
static uint32_t
fd_local2global (uint32_t local_fd) {
...
int32_t
sys_write (int32_t fd, const void* buf, uint32_t count) {
  if (fd < 0) {
 ...
 */
int32_t
sys_read (int32_t fd, void* buf, uint32_t count) {
  if (fd < 0) {
...
int32_t
sys_lseek (int32_t fd, int32_t offset, uint8_t whence) {
  if (fd < 0) {
...
/* 获得父目录的inode编号 */
static uint32_t
get_parent_dir_inode_nr (uint32_t child_inode_nr, void* io_buf) {
...
static int
get_child_dir_name (uint32_t p_inode_nr, uint32_t c_inode_nr, char* path,
                    void* io_buf) {
  ...
  if (parent_dir_inode
          ->i_sectors[12]) { // 若包含了一级间接块表,将共读入all_blocks.
...
/* 把当前工作目录绝对路径写入buf, size是buf的大小.
 ...
 失败则返回NULL */
char*
sys_getcwd (char* buf, uint32_t size) {
